...
// Generated by metaharness — the `agentic-voice-agent` CLI entry point.
//
// This is plain ESM JavaScript on purpose: it runs as-is via `npx agentic-voice-agent`
...
// "bin" + "files" fields in package.json), so `npx agentic-voice-agent` works the moment
...
const HARNESS_NAME = 'agentic-voice-agent';

/** `agentic-voice-agent init` — boot the kernel + host adapter and report status. */
async function init() {
...
/** `agentic-voice-agent doctor` — verify the install end-to-end (kernel + host resolve). */
...
export async function run(argv) {
  ...
  if (['status', 'test', 'benchmark', 'mcp'].includes(cmd)) {
    ...
    return await new Promise((done) => {
      const child = spawn(process.execPath, [resolve(root, config.entrypoint), cmd], {cwd: root, stdio: 'inherit'});
